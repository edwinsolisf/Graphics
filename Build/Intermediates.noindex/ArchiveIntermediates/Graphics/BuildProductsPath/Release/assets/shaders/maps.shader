...
void main()
{
    vec3 texColor = vec3(texture(u_MaterialDiffuseTexture, v_TexCoord));
    vec3 ambient = u_LightAmbientColor * texColor;
    
    vec3 lightDir = normalize(u_LightPosition - v_Position);
    vec3 diffuse = u_LightDiffuseColor * (max(dot(v_Normal, lightDir), 0.0) * texColor);
